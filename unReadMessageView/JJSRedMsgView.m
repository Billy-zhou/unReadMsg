...
@implementation JJSRedMsgView


-(instancetype)initWithCenterPoint:(CGPoint)centerPoint bubleRadius:(CGFloat)radius addToSuperView:(UIView *)superView{

    self = [super initWithFrame:(CGRect){{centerPoint.x - radius,centerPoint.y - radius},{radius * 2 , radius * 2}}];
...
- (void)configView {
    ...
    self.unReadMsgLabel = [[UILabel alloc] initWithFrame:self.frontView.bounds];
    self.unReadMsgLabel.textAlignment = NSTextAlignmentCenter;
    self.unReadMsgLabel.textColor = [UIColor whiteColor];
    self.unReadMsgLabel.font = [UIFont systemFontOfSize:11 weight:0.05];
    [self.frontView addSubview:self.unReadMsgLabel];
}


- (void)addBubbleAnimation {
    
    //关键帧动画CAKeyframeAnimation，在指定的时间内依次显示数组中的每一个关键帧
...
-(void)panAction:(UITapGestureRecognizer *)tapges{

    CGPoint panPoint = [tapges locationInView:self.superview];
    if (tapges.state == UIGestureRecognizerStateBegan) {
    ...
    } else if (tapges.state == UIGestureRecognizerStateEnded || tapges.state == UIGestureRecognizerStateCancelled || tapges.state == UIGestureRecognizerStateFailed) {
        ...
        if (radius_B >= radius_A / 10) {
            ...
            if (self.cleanMsgBlock) {
                self.cleanMsgBlock(NO);
            }
        } else {
            [self hidenRedMsgView];
            self.frontView.frame = self.frame;
            [self addBubbleAnimation];
            if (self.cleanMsgBlock) {
                self.cleanMsgBlock(YES);
            }
        }
...
- (void)setMsgViewColor:(UIColor *)msgViewColor{

    if (_msgViewColor != msgViewColor) {
        _msgViewColor = msgViewColor;
        self.frontView.backgroundColor = self.msgViewColor;
        self.backView.backgroundColor = self.msgViewColor;
    }
}


- (void)setStretchValue:(CGFloat)stretchValue{
    ...
    if (_stretchValue != stretchValue) {
        _stretchValue = stretchValue * 50;
    }
}
...
- (void)calculatePoint {
    ...
    radius_B = self.frontView.bounds.size.width * .5 - center_distance / self.stretchValue;
    ...
    self.animationLayer.fillColor = self.msgViewColor.CGColor;
